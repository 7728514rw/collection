...
Three native macOS apps (SwiftUI, no Xcode project) built from the same sources:

- **Game Collection** — every game you own, by system
- **Music Collection** — CDs, vinyl, cassettes, by format and artist
- **Movie Collection** — DVDs, Blu-rays, 4K discs and box sets, by format and director
...
## Build
    ...
    ./build.sh                    # builds all three into build/
    ./build.sh --install          # also copies to /Applications
    ./build.sh --install --launch # and opens them

Needs the Xcode Command Line Tools (`swiftc`), macOS 14+. `-D MUSIC` selects the music build and
`-D MOVIES` the movie build (`Sources/Kind.swift` holds the configurations: name, presets, search
hint, conditions).
...
- `~/Library/Application Support/Movie Collection/collection.json`
